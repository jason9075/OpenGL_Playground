...
float sdfSphere(vec3 p, vec3 center, float radius) { return length(p - center) - radius; }
...
vec3 getNormal(vec3 p) {
  ...
  vec3 n = vec3(sdfFunc(p, sdfCenter, sdfSize) - vec3(sdfFunc(p - e.xyy, sdfCenter, sdfSize),  // gradient in x
